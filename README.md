...
If the upstream API returns no products, the catalog renders a centered empty-state message instead of substitute data. If the upstream request fails, the route error UI provides a retry action and the original failure is logged in Vercel Runtime Logs.
